<script lang="ts">
	...
	$effect(() => {
		...
		// Update the value
		chartData = arrayValue.value.map((record) => {
                ...
                {#key chartData}
                    ...
                    {#if config.type === 'bar'}
                        <BarChart
                            ...
                            x={xKey}
                            series={activeSeries}
                            props={{ bars: { class: 'bar-chart-bar', motion: { type: 'tween', duration: 500 } } }}
                        >
                                    ...
                                    {#snippet children({ data })}
                                        ...
                                        <Tooltip.List>
                                            {#each activeSeries as s (s.key)}
                                                <Tooltip.Item
                                                    label={s.label || capitalizeStrings(s.key)}
                                                    value={data[s.key]}
                                                    color={s.color}
                                                />
                                            {/each}
                                        ...
                                        <Tooltip.List>
                                            {#each activeSeries.filter((s) => s.key === data._seriesKey) as s (s.key)}
                                                <Tooltip.Item
                                                    label={s.label || capitalizeStrings(s.key)}
                                                    value={data.value}
                                                    color={context.cScale?.(context.c(data)) || s.color}
                                                />
                                            {/each}
                                        </Tooltip.List>
                                    {/snippet}
